...
public class ALNMDeviationMetric extends PowerBandMetric {
  ...
  private static final Logger logger = LoggerFactory
      .getLogger(asl.seedscan.metrics.ALNMDeviationMetric.class);
  ...
  public void process() {
    ...
    for (Channel channel : channels) {
      ...
      ByteBuffer digest = metricData
          .valueDigestChanged(channel, createIdentifier(channel), getForceUpdate());
    ...
    if (getMakePlots() && (plotMaker != null)) {
      ...
      for (int iPanel = 0; iPanel < 3; iPanel++) {
        try {
          ...
          if (getAHNM().isValid()) {
            plotMaker.addTraceToPanel(
                new Trace(getAHNM().getPeriods(), getAHNM().getPowers(), "AHNM", Color.black,
                    stroke),
                iPanel);
          }
  ...
  private void makePlots(Channel channel, String day, double[] xdata, double[] ydata)
    ...
    if (xdata.length != ydata.length) {
      throw new MetricException(String
          .format("day=%s makePlots(): xdata.len=%d != ydata.len=%d", day, xdata.length,
              ydata.length));
    }
    if (plotMaker == null) {
      String date = String.format("%04d%03d", metricResult.getDate().getYear(),
          metricResult.getDate().getDayOfYear());
      final String plotTitle = String
          .format("[ Date: %s ] [ Station: %s ] ALNM-Deviation", date, getStation());
  ...
  /**
   * Gets the new high NoiseModel. <br> If the AHNM is not already initialized it calls {@link
   * #initAHNM()}
   *
  ...
  /**
   * Gets the new low NoiseModel. <br> If the ALNM is not already initialized it calls {@link
   * #initALNM()}
   *
  ...
  static class NoiseModel {
    ...
    /**
     * True if valid, False if not. Set in {@link asl.seedscan.metrics.ALNMDeviationMetric#readNoiseModel(XYSeries,
     * NoiseModel)}
     */
