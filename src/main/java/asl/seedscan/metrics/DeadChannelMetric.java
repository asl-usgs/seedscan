...
/**
 * DeadChannelMetric - Computes Difference (over 4-8 second period) between the the power spectral
 * density (psd) of a channel and the NLNM if this value is at or below a -7dB threshold the channel
 * is dead
 */
...
public class DeadChannelMetric extends PowerBandMetric {
  ...
  public void process() {
    ...
    for (Channel channel : channels) {
      ...
      if (digest == null) {
        logger.info("Digest unchanged station:[{}] channel:[{}] day:[{}] --> Skip metric", station,
            channel,
            getDay());
        continue;
      }
