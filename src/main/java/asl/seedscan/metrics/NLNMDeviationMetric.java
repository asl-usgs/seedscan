...
public class NLNMDeviationMetric extends PowerBandMetric {
    ...
    if (getMakePlots() && (plotMaker != null)) {
      ...
      for (int iPanel = 0; iPanel < 3; iPanel++) {
        try {
          ...
          if (getNHNM().isValid()) {
            plotMaker.addTraceToPanel(
                new Trace(getNHNM().getPeriods(), getNHNM().getPowers(), "NHNM", Color.black,
                    stroke),
                iPanel);
          }
  ...
  private void makePlots(Channel channel, String day, double[] xdata, double[] ydata)
    ...
    if (plotMaker == null) {
      ...
      final String plotTitle = String
          .format("[ Date: %s ] [ Station: %s ] NLNM-Deviation", date, getStation());
  ...
  /**
   * Gets the new high NoiseModel. <br> If the NHNM is not already initialized it calls {@link
   * #initNHNM()}
   *
  ...
  /**
   * Gets the new low NoiseModel. <br> If the NLNM is not already initialized it calls {@link
   * #initNLNM()}
   *
  ...
  static class NoiseModel {
    ...
    /**
     * True if valid, False if not. Set in {@link asl.seedscan.metrics.NLNMDeviationMetric#readNoiseModel(XYSeries,
     * NoiseModel)}
     */
