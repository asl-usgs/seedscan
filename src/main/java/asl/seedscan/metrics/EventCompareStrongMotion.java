...
public class EventCompareStrongMotion extends Metric {
  ...
  public void process() {
    ...
    if (eventCMTs == null) {
      logger.info("No Event CMTs found for Day=[{}] --> Skip EventCompareStrongMotion Metric",
          getDay());
      return;
    }
        ...
        if (srateX == srateY) {
          ...
          try { // getFilteredDisplacement
            ...
            for (String key : eventKeys) {
              ...
              double[] baseData = metricData
                  .getFilteredDisplacement(units, baseChannel, eventStartTime,
                      eventEndTime, FREQUENCY1, FREQUENCY2, FREQUENCY3, FREQUENCY4);
              double[] channelData = metricData
                  .getFilteredDisplacement(units, curChannel, eventStartTime,
                      eventEndTime, FREQUENCY1, FREQUENCY2, FREQUENCY3, FREQUENCY4);
  ...
  private double getCorr(double[] data1, double[] data2, int n1, int n2) {
    ...
    if (n2 < n1) {
      logger
          .error("station=[{}] day=[{}]: calcDiff: n2 < n1 --> Bad window", getStation(), getDay());
      return NO_RESULT;
    }
