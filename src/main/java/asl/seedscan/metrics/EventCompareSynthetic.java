 ...
 * </p>
 * <p>
 * The difference is calculated by a power scale formula result=SUM(data[i] * syn[i]) / SUM(syn[i] *
 * syn[i])
 * <p>
 * Result meanings<br> x = 0 data is to small or all 0s. The channel is dead.<br> {@literal 0 < x <
 * 1} data shows less displacement than the synthetic.<br> 1 = data aligns exactly with the
 * synthetic.<br> {@literal x > 1} data shows greater displacement than the synthetic.<br> {@literal
 * x < 0} The data is out of phase from the synthetic.<br> x = -1 The data is exactly 180 degrees
 * out of phase, but matches the data.
 * <br>
 * </p>
 *
 * <a href="http://srl.geoscienceworld.org/content/77/1/12.full">Observations of
 * Time-dependent Errors in Long-period Instrument Gain at Global Seismic Stations</a>Equation 3
 */
public class EventCompareSynthetic extends Metric {
        ...
        try { // getFilteredDisplacement
          ...
          for (String key : eventKeys) {
            ...
            String fileKey = getStn() + "." + basechannel[0] + "." + basechannel[1].substring(0, 2)
                + channelVal.charAt(2) + ".modes.sac.proc";
   ...
   * Sac array to double.
   *
   * @param sac the sac timeseries data
  ...
  /**
   * Compare 2 double[] arrays between array indices n1 and n2 currently doing: SUM[ x(n) * y(n) ] /
   * SUM[ y(n) * y(n) ], where x(n)=data and y(n)=synth
   * <p>
   * {@literal difference = 0. -->} data are all zero<br> {@literal difference = 1. -->} data
   * exactly matches synthetic<br> {@literal difference = 1. -->} data exactly matches -synthetic
   * (is 180 deg out of phase)<br>
   *
   ...
   * data1 = x, data2 = y
   *
   * @param data1 the data in meters displaced
   * @param data2 the synthetic in meters displaced
   * @param n1    the window start; nstart in process()
   * @param n2    the window end; nend in process()
   * @return the result
   */
