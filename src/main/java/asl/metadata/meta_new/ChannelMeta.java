...
/**
 * A ChannelMeta consists of a series of ResponseStages. Typically there will be 3 ResponseStages,
 * numbered 0, 1 and 2. ResponseStages 0 and 2 will likely contain only gain and frequencyOfgain
 * info (e.g., Blockette B058) ResponseStage 1 will contain this info + specific instrument response
 * (e.g., PoleZero, Polynomial) info, so that the complete channel response can be obtained by
 * scaling ResponseStage1 response by the gains in ResponseStage1 and ResponseStage2.
 * <p>
 * In the future, we may wish to read in higher Stages (3, 4, ...) that are 'D'igital stages with
 * non-zero coefficients (numerator and denominator), so DigitalStage has been left general in order
 * to be able to read and store these coefficients.
 *
...
public class ChannelMeta extends MemberDigest implements Serializable,
  ...
  private boolean invalidResponse() {
    ...
    if (isSeismicChannel) {
      if (!hasStage(0) || !hasStage(1) || !hasStage(2)) {
        logger.error(String
            .format(
                "invalidResponse: All Stages[=0,1,2] have NOT been loaded for chan-loc=%s-%s date=%s\n",
      ...
      if (stageGain0 <= 0 || stageGain1 <= 0 || stageGain2 <= 0) {
        logger.error(String
            .format(
                "invalidResponse: Gain =0 for either stages 0, 1 or 2 for chan-loc=%s-%s date=%s\n",
                this.getLocation(), this.getName(),
                this.getDate()));
        return true;
      }
    ...
    if (!(stage instanceof PoleZeroStage)) {
      ...
      if (outUnits != 0) { // Convert response to desired responseOut Units
        ...
        if (inUnits == 0) {
          String msg = String
              .format(
                  "getResponse:[%s] date:[%s] Response requested but PoleZero Stage Input Units = Unknown!",
  ...
  /**
   * processEpochData Convert EpochData = {@literal Hashtable<StageNumber, StageData>} for this
   * Channel + Epoch Into a sequence of ResponseStages, one for each StageNumber For now we're just
   * pulling/saving the first 3 stages
   * <p>
   * For each stageNumber, check for a B058 and if present, grab Gain + freqOfGain Then, if you see
   * a B054 {@literal --> create a new DigitalStage & add to} ChannelMeta else, if you see a B053
   * {@literal --> create a new PoleZeroStage & add} to ChannelMeta else ...
   **/
      ...
      if (epochData.hasStage(stageNumber)) {
        ...
        if (stage.hasBlockette(58)) {
          ...
          if ((stageNumber != 1) && !(stage.hasBlockette(54))) {
            ...
            if (stageNumber != 0) {
              logger.warn(
                  "== Warning: MetaGenerator: [{}_{} {}-{}] date:{} stage:{} has NO Blockette B054",
                  station.getNetwork(),
        ...
        if (stage.hasBlockette(54)) {
          ...
          DigitalStage digitalStage = new DigitalStage(stageNumber, stageType[0], Gain,
              frequencyOfGain);
