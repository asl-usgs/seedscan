...
/**
 * Our internal representation of a station's metadata Holds all channel metadata for a single
 * station for a specified day.
 *
...
public class StationMeta implements Serializable {
  ...
  /**
   * Whitelist for locations that are needed to have a data re-run based on metadata changes. This
   * is used to restrict scans to only locations that have had their metadata updated. When any
   * channel in a location gets its metadata updated then all locations should change. This is based
   * on a listing from an IRIS service of metadata changes; see https://service.iris.edu/irisws/metadatachange/1/
  ...
  /**
   * Look for particular channelMeta (e.g., "00" "VHZ") in channels Hashtable. Return it if found,
   * else return null
   *
   * @param chanKey the channel key
  ...
  /**
   * Return ArrayList of all channels contained in this stationMeta that match the band e.g., if
   * band = "LH" then return "00-LHZ", "00-LH1", "00-LH2", "00-LHN", "00-LHE", "10-LHZ", "10-LH1",
   * "10-LH2", .. -or- "---LHZ", "---LH1", "---LH2", "---LHN", "---LHE"
   * <p>
   * This methods assumes that Non derived channels are always 3 chars in length e.g. LH*
   *
   * @param bands           the frequency band
   * @param ignoreTriggered false includes triggered channels, true restricts it to continuous.
   * @param ignoreDerived   false includes derived channels, true ignores derived
   * @return the channel array
   */
  public List<Channel> getChannelArray(String bands, boolean ignoreTriggered,
   ...
   * then all channels are part of the whitelist. If a channel at this channel's location has had a
   * metadata change, then it should be part of any rescans. (For example, if ANMO 00-LHZ has had
   * its sensitivity changed, then we should redo any scans on 00-LH1
   *
  ...
  /**
   * Convert 1/2 component descriptions to matching N/E or 1/2 channels depending on which actually
   * exists. All components except 1/2 are ignored and return null.
   *
   * @param location  the location code EG 00
   * @param band      the frequency band EG LH
   * @param component the component restricted to 1/2. E/N are not allowed
   * @return the existing channel that corresponds to the passed 1 or 2 component. Null if none
   * exists or not 1/2 component.
   */
  ...
  /**
   * Return a ArrayList of channels that are able to be derived. Actual rotation occurs in
   * MetricData, when a hash digest fails to be computed. These channels have names like LHED, BHND,
   * HHZ, the 'D' standing for derived.
   *
  ...
  /**
   * Checks that a channel triplet exists. E.G. LHZ, LHE, LHN  or LHZ,LH1,LH2
   *
   * @param location the location
  ...
  /**
   * Not sure yet if we need this to drive addRotatedChannel below e.g., if we need to rotate 2
  ...
  /**
   * If handed a derivedChannelName like "LHND" (or "LHED"), try to create it from LH1 (or LH2)
   * channel metadata. If these aren't found, look for LHN (or LHE) instead.
   *
   * @param location           the location
