...
/**
 * MetaGenerator - Holds metadata for all networks x stations x channels x epochs Currently reads
 * metadata in from network dataless seed files
 *
...
public class MetaGenerator {
  ...
  static String getStationNameFromPath(String filePath, String networkName,
      String datalessFilePattern) {
  ...
  static List<String> getDatalessFilesForNetwork(File datalessDir, String datalessFilePattern,
      String networkName) {
  ...
  SeedVolume buildVolumesFromStringData(List<String> strings, String networkName,
      String stationName) throws DatalessParseException {
  ...
  /**
   * loadDataless() reads in the entire dataless seed file (all stations) getStationData returns the
   * metadata for a single station for all epochs It is called by {@link
   * asl.metadata.MetaGenerator#getStationMeta(Station, LocalDateTime)} below.
   *
   * @param station the station
   * @return the station data - this can be null if seed files are malformatted
   */
  private StationData getStationData(Station station) {
    ...
    if (stationData == null) {
      logger.error(
          "stationData is null ==> This COULD be caused by incorrect network code INSIDE seedfile ...");
    }
    return stationData;
  }
