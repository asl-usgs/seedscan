...
public class SeedVolume {
  ...
  /**
   * The network key. This is derived from filename (i.e., XX in XX.dataless is network name) This
   * used to be derived from info blockette field 9, but this is not necessarily the name.
   */
  ...
  public SeedVolume(Blockette volumeInfo, String networkName, String stationName) {
    ...
    this.stationKey = new StationKey(networkName,
        stationName); // formerly derived from vol info blockette
    stations = new Hashtable<>();
    stationLocators = new ArrayList<>();
  }
   ...
   * @param data the station metadata
   */
   ...
   *
   * @param stationLocator the station locator
   */
