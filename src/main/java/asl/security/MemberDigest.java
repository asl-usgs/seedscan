...
public abstract class MemberDigest implements Serializable {
   ...
   * @throws RuntimeException if the passed algorithm could not be used.
   */
  ...
  /**
   * Gets the digest bytes. Calls {@link #computeDigest()} which in turn calls the abstracted {@link
   * #addDigestMembers()}
   *
   ...
   *
   * @param digests the ByteBuffer Collection
   ...
   * @throws RuntimeException if the given algorithm causes a {@linkplain java.security.NoSuchAlgorithmException}
   */
  private void setAlgorithm(String algorithm) {
    try {
      ...
      String message =
          "Could not initialize digest for the '" + algorithm + "' algorithm:" + ex.getMessage();
      throw new RuntimeException(
          message); //If this happens, something is wrong with code that called.
    }
  }
   ...
   *
   * @param in the ObjectInputStream containing the object and algorithm
   * @throws IOException            Signals that an I/O exception has occurred.
