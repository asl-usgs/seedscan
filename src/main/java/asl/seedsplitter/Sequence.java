...
public class Sequence extends MemberDigest implements Comparable<Sequence>, Serializable {
  ...
  /**
   * Adds start time of first data point and sample rate to digest. It then loops through blocks and
   * adds them.
   */
  @Override
   ...
   * @throws IndexOutOfBoundsException - if copying would cause access of data outside array
   *                                   bounds.
   * @throws ArrayStoreException       - if an element in the buffer array could not be stored
  ...
  /**
   * Swaps the data contained within two Sequences such that they now contain each other's data.
   * This is not a thread safe operation, and requires that all of the data components be
   * synchronized correctly in order to prevent any being set while this operation is in progress.
   *
  ...
  /**
   * Merges this Sequence into the target Sequence, removing all data points from this Sequence.
   * <p>
   * A SequenceIntervalMismatchException is thrown when the Sequences do not have the same
   * interval.
   * <p>
   * A SequenceMergeRangeException is thrown when the Sequences are not contiguous, or do not
   * overlap.
   *
  ...
  synchronized void mergeInto(Sequence seq)
      throws SequenceIntervalMismatchException, SequenceMergeRangeException,
    ...
    logger.debug("    Source DataSet: " + Sequence.timestampToString(this.getStartTime()) + " to "
        + Sequence.timestampToString(this.getEndTime()) + " (" + this.getLength()
        + " data points)");
    ...
    if (m_startTime > seq.getEndTime()) {
      if (((m_startTime - seq.getEndTime()) < (m_interval - intervalAdjustment))
        ...
        logger.warn("Source is more than 1 data point after target. (difference = "
            + (m_startTime - seq.getEndTime()) + " ms OR " + ((m_startTime - seq.getEndTime())
            / m_interval)
            + " data points)");
  ...
  /**
   * Returns an array of integer values that falls within the specified range. The specified start
   * and end times must be within the range of the actual data.
   *
  ...
  /**
   * Determines if this Sequence has a start time which is earlier than the start time of the
   * supplied reference Sequence (may overlap).
   *
   * @param seq The reference Sequence with which to compare this Sequence.
   * @return A boolean value; true if this Sequence starts before the start of the reference
  ...
  /**
   * Determines if this Sequence has an end time which is later than the end time of the supplied
   * reference Sequence (may overlap).
   *
   * @param seq The reference Sequence with which to compare this Sequence.
   * @return A boolean value; true if this Sequence ends after the end time of the reference
   * Sequence, otherwise false.
   */
  ...
  /**
   * Reports whether this Sequence contains data within the specified time range.
   *
  ...
  /**
   * Reports whether this Sequence is a sub-sequence of the supplied reference Sequence.
   *
   * @param seq The reference sequence with which to compare this Sequence.
   * @return A boolean value: true if the range of this Sequence is within the range of the
   * reference Sequence.
   * @throws SequenceIntervalMismatchException if the reference Sequence's frequency does not match
   *                                           that of this Sequence.
   */
   ...
   * @throws IllegalSampleRateException if the supplied sample rate is not one of the accepted
   *                                    values.
   */
  ...
  /**
   * Converts a microsecond granularity timestamp into a human readable string.
   *
   * @param timestamp - microseconds since the Epoch (January 1, 1970 00:00:00.000000 GMT)
   * @return Returns the timestamp as a human readable string of the format YYYY/MM/DD
   * HH:MM:SS.mmmmmm
   */
   ...
   * <p>
   * It compares the start and end times. If they all match then returns 0. The priority is with
   * starts. If these match it compares end times.
   *
   * @param other the sequence to compare to
   * @return An integer value: 0 if Sequences are considered equal; less than 0 if this Sequence
   * comes before Sequence other; greater than 0 if Sequence other comes before this Sequence.
