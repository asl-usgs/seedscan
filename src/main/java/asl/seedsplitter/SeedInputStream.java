 ...
 * <p>
 * Reads MiniSEED records from multiple files, and pushes them into a queue to be processed by a
 * supported class (seed {@link SeedSplitProcessor}).
 */
public class SeedInputStream implements Runnable {
  ...
  /**
   * Reads data from the input stream, assembles full SEED records and pushes them into the queue
   * for processing.
   */
  @Override
  public void run() {
          ...
          if (m_bufferBytes == BLOCK_SIZE) {
            ...
            if (
                qualityFlags.contains("All") ||
                    qualityFlags
                        .contains(String.valueOf((char) indicator)) // converts from int to String
            ) {
              try {
