...
/**
 * This class represents a mini-seed packet. It can translate binary data in a byte array and break
 * apart the fixed data header and other data blockettes and represent them as separate internal
 * structures.
 *
...
public class MiniSeed {
  ...
  /**
   * these wrap the bufnnn below for each blockette found in same order as the blocketteList
   */
  ...
  /**
   * These bufnnn contain data from the various possible "data blockettes" found. They are never all
   * defined.
   */
  private byte[] buf100;
  ...
  /**
   * ms set the number of samples. Normally used to truncate a time series say to make it just long
  ...
  /**
   * Is this mini-seed a heart beat. These packets have all zero sequence # and all spaces in the
  ...
  /**
   * This returns the time data as a 4 element array with hour,minute, sec, and hsec from a raw
   * miniseed buffer in buf. This routine would be used to extract a bit of data from a raw buffer
  ...
  /**
   * This returns the digitizing rate from a raw miniseed buffer in buf. This routine would be used
   * to extract a bit of data from a raw buffer without going to the full effort of creating a
   * MiniSeed object from it.
   *
   * @param buf A array with a miniseed block in raw form
  ...
  /**
   * This returns the seedname in NSCL order from a raw miniseed buffer in buf. This routine would
   * be used to extract a bit of data from a raw buffer without going to the full effort of creating
   * a MiniSeed object from it.
   *
   * @param buf A array with a miniseed block in raw form
  ...
  private static Boolean swapNeeded(byte[] buf, ByteBuffer bb)
    ...
    if ((buf[0] >= '0' && buf[0] <= '9') && (buf[1] >= '0' && buf[1] <= '9') && (buf[2] >= '0'
        && buf[2] <= '9') && (buf[3] >= '0' && buf[3] <= '9') && buf[7] == ' ' &&
        (
      ...
      if (nblks > 0) {
        ...
        if (offset > 64 || offset < 48) { // This looks like swap is needed
          ...
          if (offset > 200 || offset < 0) {
            datalogger.error(
                "MiniSEED: cannot figure out if this is swapped or not!!! Assume not. offset="
   ...
   * Crack block size.
   * <p>
   * This method attempts to determine the block size in blockette 1000. If a record has field 17
   * (position 44) is 0 it is changed to 64. This prevents issues with CU stations having OCF
   * channels that are invalid.
   *
   * @param buf the seed buffer
   * @return the block size
   * @throws IllegalSeednameException occurs if the blockette has an in valid name or channel. This
   *                                  can only occur if a BlockSizeException has happened already.
   * @throws BlockSizeException       occurs if either a blockette Offset is too small or too large
  ...
  public static int crackBlockSize(byte[] buf)
    ...
    if (dataOffset == 0) {
      logger.warn(
          "Data Offset is 0. Either there is no data or there is a problem. Treating as if it was 64.");
  ...
  /**
   * this routine takes a binary buf and breaks it into the fixed data header and any other data
   * blockettes that are present. Note : creating a miniseed object does nothing but store the data.
   * Anything that needs to use the "cracked" data structures should call crack first. If the record
  ...
  /**
   * create a basic string from the fixed data header representing this packet. any other data
   * blockettes are added at the end to indicate their presence
   *
  ...
  /**
   * return number of used data frames. We figure this by looking for all zero frames from the end!
   *
  ...
  /**
   * return state of swap as required by the Steim decompression routines and our internal
   * convention. We both assume bytes are in BIG_ENDIAN order and swap bytes if the records is not.
   * Stated another way, we swap whenever the compressed data indicates it is is little endian
   * order.
   *
  ...
  /**
   * return a GregorianCalendar set to the 1st sample time, it is a copy just so users do not do
   * calculations with it and end up changing the mini-seed record time. This time is rounded in
   * millis
   *
  ...
  /**
   * return a GregorianCalendar set to the 1st sample time, it is a copy just so users do not do
   * calculations with it and end up changing the mini-seed record time. This time is truncated in
   * millis
   *
   * @return The 1st sample time
   */
  ...
  /**
   * get the raw buffer representing this Miniseed-record. Beware you are getting the actual buffer
   * so changing it will change the underlying data of this MiniSeed object. If this is done after
   * crack, some of the internals will not reflect it.
   *
  ...
  /**
   * return the record length from the blockette 1000. Note the length might also be determined by
   * getBuf().length as to how this mini-seed was created (buf is a copy of the input buffer).
   *
